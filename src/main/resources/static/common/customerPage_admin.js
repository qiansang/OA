...
var idealist,sourcelist, titlelist,professionList,safeList,userList;
var success = 0;
...
$(function () {
    ...
    $.ajax({
        ...
        success:function (data) {
            if(data.code == 200){
                var html = "<a id='user_0' onclick='getUserId(0)'>不限</a>";
                userList = eval(data.data);
                $("#manager").html();
                for(var i = 0; i < userList.length; i++){
                    html += "&nbsp;&nbsp;<a  id='user_" + userList[i].id + "' onclick='getUserId(" + userList[i].id + ", 0)'>" + userList[i].name + "</a>";
                }
                $("#manager").html(html);
            }else{
                layer.alert(data.msg);
    ...
    var timeJob = setInterval(function(){
        if(idealist==undefined || sourcelist == undefined || titlelist == undefined || professionList == undefined || safeList == undefined || userList == undefined){
            return false;
        }else{
            getTableInfo();
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            , cols: [[
                ...
                ,{field:'user', width:80, title: '业务员',templet: function(res){
                        var html = "<div class='" + res.id + "'><select name='user' id='user_table' lay-filter='user_table' class='layui-form-select'>";
                        if(res.user == null){
                            ...
                            for (var i = 0; i < userList.length; i++) {
                                if(res.userId == userList[i].id){
                                    html += "<option value='" + userList[i].id + "' selected='selected'>" + userList[i].name + "</option>";
                                }else{
                                    html += "<option value='" + userList[i].id + "'>" + userList[i].name + "</option>";
                                }
                            }
...
function editCompanyInfo(res) {
    ...
    layui.use(['form', 'laydate', 'table'], function () {
        ...
        html += "                </select>\n"+
            ...
            "                <button class=\"layui-btn layui-btn-primary\" id=\"quxiao4\">取消</button>\n" +
            "            </div>\n" +
            "        </div>\n" +
            "    </form>";
        $("#userModel").html(html)
...
layui.use(['form', 'table'], function () {
    ...
    $("#delete").click(function () {
        var checkStatus = table.checkStatus('table_test');
        if(checkStatus.data.length == 0){
            layer.msg('请选择要删除的客户信息.', {icon: 2});
            return;
        }
